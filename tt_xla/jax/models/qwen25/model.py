...
def apply_rotary_emb(
    ...
    Args:
        query_states: Query tensor of shape [batch, seq_len, n_heads, head_dim]
        key_states: Key tensor of shape [batch, seq_len, n_kv_heads, head_dim]
    ...
    """
    # Log shapes for detailed debugging
    logger.debug(f"apply_rotary_emb: query_states shape={query_states.shape}")
    logger.debug(f"apply_rotary_emb: key_states shape={key_states.shape}")
    logger.debug(f"apply_rotary_emb: cos shape={cos.shape}")
    logger.debug(f"apply_rotary_emb: sin shape={sin.shape}")
    
    # Get dimensions
    q_batch, q_seq, q_heads, q_dim = query_states.shape
    k_batch, k_seq, k_heads, k_dim = key_states.shape
    
    # Special handling for different dimensions of rotary embeddings
    # The problem is that cos/sin might have a dimension of 128, while the query head_dim is 64
    # In this case, we need to slice the cos/sin tensors
    
    # First, ensure cos and sin have compatible dimensions
    if cos.shape[-1] > q_dim:
        # Slice the cos/sin tensors to match the query dimension
        logger.info(f"Slicing cos/sin from shape {cos.shape} to match query dimension {q_dim}")
        cos = cos[..., :q_dim]
        sin = sin[..., :q_dim]
        logger.debug(f"After slicing: cos shape={cos.shape}, sin shape={sin.shape}")
    
    # Process query states
    # Split query into real and imaginary parts
    ...
    # Now reshape using the actual dimensions
    ...
    # Reshape cos/sin for broadcasting with key
    cos_for_k = cos_for_key.reshape(1, cos_for_key.shape[0], 1, key_cos_feature_dim)
    sin_for_k = sin_for_key.reshape(1, sin_for_key.shape[0], 1, key_cos_feature_dim)
    ...
    def __call__(
        ...
        try:
            if position_ids is not None:
                ...
                max_pos = jnp.max(position_ids) + 1
                ...
                # Rather than using a larger dimension, generate exactly what we need
                # to avoid dimension mismatches later
                # Note: We need to generate for head_dim (not head_dim//2) to ensure
                # we have the correct size for the complex representation
                ...
                # Make sure we're creating embeddings that can be properly reshaped later
                # Each position needs head_dim/2 pairs of (cos, sin) values
                ...
                # Slice embeddings based on position_ids if needed
                if cos.shape[0] > jnp.max(position_ids) + 1:
                ...
                feature_dim = head_dim
                freqs = 1.0 / (rope_theta ** (jnp.arange(0, feature_dim // 2, dtype=jnp.float32) / (feature_dim // 2)))
                t = jnp.arange(seq_length, dtype=jnp.float32)
                freqs = jnp.outer(t, freqs)
...
class Qwen25ForCausalLM(nn.Module):
    ...
    def __call__(
        ...
        logits = self.lm_head(hidden_states, precision=jax.lax.Precision.DEFAULT)
